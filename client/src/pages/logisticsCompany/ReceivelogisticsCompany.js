...
export default function ReceivelogisticsCompany(props) {
  ...
  const navItem = [
    ["Receive Product", "/logisticsCompany/receive"],
    ["Ship Product", "/logisticsCompany/ship"],
  ];
  ...
  const handleReceiveButton = async (id, long, lat) => {
    try{
      await supplyChainContract.methods
      .receiveBylogisticsCompany(parseInt(id), long, lat)
      .send({ from: roles.logisticscompany, gas: 1000000 })
  ...
  return (
    <div classname={classes.pageWrap}>
      <Navbar pageTitle={"logistics Company"} navItems={navItem}>
        {loading ? (
          <Loader />
        ) : (
          <>
