...
export default function ReceiveAutoDealer(props) {
  ...
  const navItem = [
    ["Buy Product", "/AutoDealer/allProducts"],
    ["Receive Product", "/AutoDealer/receive"],
    ["Ship Products", "/AutoDealer/ship"],
  ];
  React.useEffect(() => {
    (async () => {
      setLoading(true);
  ...
  const handleReceiveButton = async (id, long, lat) => {
    try{
      ...
      .receiveByAutoDealer(parseInt(id), long, lat)
      .send({ from: roles.autodealer, gas: 1000000 })
  ...
  return (
    <div classname={classes.pageWrap}>
      <Navbar pageTitle={"Auto Dealer"} navItems={navItem}>
        {loading ? (
          <Loader />
        ) : (
          <>
